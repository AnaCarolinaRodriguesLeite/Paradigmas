...
paisClube(gladbach,alemanha).
paisClube(leipzig,alemanha).
paisClube(mainz,alemanha).
paisClube(schalke,alemanha).
paisClube(stuttgart,alemanha).
paisClube(frankfurt,alemanha).
paisClube(freiburg,alemanha).
paisClube(werderBremen,alemanha).

paisClube(alNassr,arabia).
...
paisClube(alajuelense,costaRica).
paisClube(herediano,costaRica).
paisClube(saprissa,costaRica).

paisClube(jeonbukMotors,coreiaDoSul).
paisClube(ulsan,coreiaDoSul).
...
paisClube(atleticoDeMadrid,espanha).
paisClube(barcelona,espanha).
...
paisClube(psv, holanda).

paisClube(arsenal, inglaterra).
paisClube(astonVilla, inglaterra).
paisClube(brighton, inglaterra).
paisClube(chelsea, inglaterra).
...
paisClube(atalanta, italia).
paisClube(fiorentina, italia).
paisClube(inter, italia).
paisClube(juventus, italia).
...
paisClube(nacional,uruguai).

paisClube(redStar,servia).